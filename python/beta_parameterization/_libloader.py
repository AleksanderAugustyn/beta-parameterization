"""Locate and load libbeta_parameterization.so for ctypes."""
...
_LIB_NAME = "libbeta_parameterization.so"


def _candidate_paths() -> Iterator[Path]:
    env = os.environ.get("BETA_PARAM_LIB")
    if env:
        yield Path(env)
    # Installed-wheel location: the .so is bundled next to this file.
    yield Path(__file__).resolve().parent / _LIB_NAME
    # Dev fallback: this file is <repo>/python/beta_parameterization/_libloader.py
...
def load_library() -> ctypes.CDLL:
    """Load the shared library, trying BETA_PARAM_LIB, build dirs, then the system path."""
    ...
    raise OSError(
        ...
        f"BETA_PARAM_LIB to its full path. Last error: {last_error}"
    )
